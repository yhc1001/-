...
with col2:
    if st.session_state.equipments:
        ...
        # [수정] ranksep을 늘려 위아래 너비 확보, ordering='in'으로 순서 고정
        dot.attr(rankdir='TB', splines='ortho', fontname='NanumGothic', nodesep='0.3', ranksep='0.8')
        ...
        with dot.subgraph(name='cluster_legend') as l:
            l.attr(label='', style='invis', rankdir='LR') 
            l.node('LEG_W', 'W: 전력량계', shape='circle', width='0.25', height='0.25', fontsize='8')
            l.node('LEG_R', 'R: RTU', shape='square', width='0.25', height='0.25', fontsize='8')
        ...
        # 4. 설비 클러스터 (ordering='in'으로 순서 고정)
        with dot.subgraph(name='cluster_equip') as c:
            c.attr(style='dashed', color='gray', ordering='in')
            ...
            for i, eq in enumerate(st.session_state.equipments):
                ...
                if eq['has_w']:
                    w_id = f'W_{i}'
                    c.node(w_id, 'W', shape='circle', width='0.25', height='0.25')
                    c.edge(w_id, eq_id)
